...
function App() {
  ...
  // Chain parameters for wallet switching
  const networkParams = {
    Base: { chainId: "0x2105" }, // Base Mainnet
    Celo: { chainId: "0xa4ec" }, // Celo Mainnet
    Optimism: { chainId: "0xa" }, // Optimism Mainnet
    Sepolia: { chainId: "0xaa36a7" } // Sepolia Testnet
  };

  // Switch wallet network when dropdown changes
  async function handleNetworkChange(e) {
    const selected = e.target.value;
    setNetwork(selected);
    if (window.ethereum && networkParams[selected]) {
      try {
        await window.ethereum.request({
          method: "wallet_switchEthereumChain",
          params: [{ chainId: networkParams[selected].chainId }]
        });
      } catch (err) {
