...
import { AbiContract, HomeContract, HowItWorksContract, MyDeploymentsContract, SimpleStorageContract, ClickCounterContract, MessageBoardContract, SimpleVotingContract, contractsList } from '../components/Contracts.jsx';
    ...
    <div>
      ...
      {showHeader && (
        <div className="header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '28px', padding: '18px 40px', position: 'fixed', top: 0, left: 0, width: '100%', zIndex: 1000, background: theme.gradient }}>
          <span className="header-title" style={{ color: theme.accent || '#fff', fontFamily: 'Inter, Arial, sans-serif', fontWeight: 700, fontSize: '1.8em' }}>
            Contract Deployer
          </span>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <span style={{ color: theme.accent || '#fff', fontWeight: 600 }}>Chain</span>
            <div style={{ position: 'relative', minWidth: 120 }}>
              <div style={{ padding: '6px 18px', borderRadius: 8, background: '#FFE000', color: '#2D3A29', cursor: 'pointer' }} onClick={() => setShowDropdown && setShowDropdown(prev => !prev)}>{network}</div>
              {showDropdown && (
                <div style={{ position:'absolute', top:'110%', left:0, background: '#FFE000', borderRadius:8, zIndex:1002, minWidth:120 }}>
                  {networks.map(n => (
                    <div key={n} onClick={() => { setNetwork(n); setShowDropdown(false); handleNetworkChange({ target:{ value: n } }); }} style={{ padding:'8px 12px', cursor:'pointer', color: '#2D3A29' }}>{n}</div>
                  ))}
                </div>
              )}
            </div>
